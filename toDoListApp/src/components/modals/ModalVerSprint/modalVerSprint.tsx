...
export const ModalVerSprint: FC<IModalEditarSprint> = ({ isOpen, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (isOpen && sprintActivo){
            ...
            const opcionesSeleccionadas = sprintActivo.tareas.map(t => ({
                value: t.id,
                label: t.titulo
            }));
        
            setSelectedOptions(opcionesSeleccionadas);
        }
    }, [isOpen, sprintActivo])
    ...
    return (
        <div style={{ display: isOpen ? "" : "none" }} className={styles.background} onClick={handleCloseModal}>
            <div className={styles.modalGlobal} onClick={(e) => e.stopPropagation()}>
                <h2 className={styles.modalTitle} style={{ color: "#FBFF80" }}>Ver Sprint</h2>
                <button className={styles.closeButton} onClick={handleCloseModal}>✖</button>
                <form>
                    <div>
                        <div>
                            <p className={styles.fieldTitle}>Nombre:</p>
                            <input disabled type="text" name="nombre" value={editedSprint.nombre} placeholder="Nombre:" className={styles.fieldInput} onChange={handleChangeInputs} />
                        </div>
                        <div>
                            <p className={styles.fieldTitle}>Fecha inicio:</p>
                            <input disabled type="date" name="fechaInicio" value={editedSprint.fechaInicio} className={styles.fieldInput} onChange={handleChangeInputs} />
                        </div>
                        <div>
                            <p className={styles.fieldTitle}>Fecha cierre:</p>
                            <input disabled type="date" name="fechaCierre" value={editedSprint.fechaCierre} className={styles.fieldInput} onChange={handleChangeInputs} />
                        </div>
                        <div>
